...
<iframe 
  width="560" 
  height="315" 
  src="https://www.youtube.com/embed/uLLzEAp9DL4" 
  title="Mega CLI Demo" 
  frameborder="0" 
...
3. Or, deploy to MegaETH testnet using your account:

```bash
mega deploy foundry-app/src/GmegaCounter.sol:GmegaCounter \
  --network testnet \
  --broadcast \
...
```bash
# Check a local account balance (local is the default network)
mega balance --account <account-name>

# Check any address on MegaETH testnet
mega balance 0x123...abc --network testnet

# Check an account on MegaETH mainnet and show ETH
mega balance --account dev --network mainnet --ether
```
