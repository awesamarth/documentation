...
mega deploy src/Counter.sol:Counter --network testnet --broadcast --account <account-name>
```
...
```bash
mega deploy src/Counter.sol:Counter --network mainnet --broadcast --account <account-name>
```

You can override a network's default RPC while retaining chain ID validation:

```bash
mega deploy src/Counter.sol:Counter \
  --network mainnet \
